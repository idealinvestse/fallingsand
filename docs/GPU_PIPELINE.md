...
The simulation is GPU-first. Cells live in double-buffered SSBOs, while velocity, pressure, temperature, vorticity, display output, and auxiliary fields live in textures.

## Frame Step Order

`gpu/pipeline.py` currently dispatches these passes for each adaptive substep:
...
10. `acoustic_pressure_step.glsl` / `acoustic_velocity_step.glsl`
11. `advect_shader.glsl`

Rendering dispatches `render_shader.glsl`, then blits `display_texture` to the screen.
